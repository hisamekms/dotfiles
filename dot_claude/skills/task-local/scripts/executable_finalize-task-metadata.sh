#!/usr/bin/env bash
# Finalize task metadata by setting status=todo and depends_on/tags arrays.
# Usage:
#   ./scripts/finalize-task-metadata.sh <task_file> [--depends <id>]... [--tag <tag>]...

set -euo pipefail

task_file="${1:-}"
[ -n "$task_file" ] || { echo "usage: $0 <task_file> [--depends <id>]... [--tag <tag>]..." >&2; exit 2; }
shift || true
...
tmp="${task_file}.tmp"
awk -v depends="$depends_yaml" -v tags="$tags_yaml" '
  BEGIN { in_fm = 0 }
  /^---$/ {
    print
    if (in_fm == 1) {
