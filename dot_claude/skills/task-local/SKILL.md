...
環境変数 `TASK_LOCAL_MODE` で実行モードを切り替える。デフォルトは `normal`。

| 確認ポイント | `full-auto` | `semi-auto` | `normal` |
|---|---|---|---|
| 仕様・不明点の質問 | スキップ | 質問する | 質問する |
| プランモード（Step 3） | スキップ | 実施 | 実施 |
| タスク追加時の確認 | スキップ | 確認する | 確認する |
| 完了確認（Step 5） | スキップ | 確認する | 確認する |
| マージ・worktree削除の確認（Step 6） | スキップ | スキップ | 確認する |

- **`full-auto`**: ユーザーへの確認を一切行わず、全ステップを自動で進める。プランモードもスキップし、タスク本文を基に直接実装に入る。
- **`semi-auto`**: 仕様や不明点などユーザーの意思決定が必要な事項は質問する。プランモードも実施し承認を求める。完了確認も行う。ただし、マージ・worktree削除などの操作については確認せずに自動実行する。
- **`normal`**（デフォルト）: 全てのステップでユーザーに確認を求める。最も安全なモード。
...
5. **[`full-auto`: スキップ]** `AskUserQuestion` で以下を確認する:
   - タイトルと本文の内容が適切か
...
#### Step 3: プランモード **[`full-auto`: スキップ]**

`EnterPlanMode` を使ってタスクの実装計画を立てる。タスクファイルの本文を基に、コードベースを調査し、実装方針を検討する。

ユーザーがプランを承認するまで待つ。

以降のStep 5〜7 はプランファイルに盛り込むこと。Step 7 のタスク完了更新にはタスクファイルの絶対パスを記載すること。

#### Step 4: タスクの実装

承認されたプランに基づいてタスクを実装する（`full-auto` の場合はタスク本文を基に直接実装する）。通常のコーディングフローに従う。

#### Step 5: 完了確認 **[`full-auto`: スキップ]**
...
#### Step 6: Worktreeの後処理 **[`normal`: 確認あり / 他: 自動実行]**
...
- **安全性**: worktreeの作成・削除、ブランチ操作は慎重に行い、実行モードに応じてユーザーの確認を得る
